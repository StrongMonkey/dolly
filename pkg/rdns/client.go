...
func (c *Client) setSecret(resp *Response) error {
	...
	_, err := c.k8s.CoreV1().Secrets("kube-system").Create(context.Background(), s, metav1.CreateOptions{})
	...
	if err != nil && k8serrors.IsAlreadyExists(err) {
		if _, err := c.k8s.CoreV1().Secrets("kube-system").Update(context.Background(), s, metav1.UpdateOptions{}); err != nil {
			return err
		}
		return nil
	}
...
func (c *Client) getSecret() (string, string, error) {
	sec, err := c.k8s.CoreV1().Secrets(c.namespace).Get(context.Background(), secretKey, metav1.GetOptions{})
	if err != nil {
		return "", "", err
	}
...
func NewClient(k8s kubernetes.Interface, namespace string) *Client {
	return &Client{
